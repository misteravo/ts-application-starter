...
export async function verifySecurityKeyAction(data: unknown): Promise<ActionResult> {
  ...
  if (!session.emailVerified || !user.registeredSecurityKey || session.twoFactorVerified) return { error: 'Forbidden' };
  ...
  // TODO: Update host
  if (!authenticatorData.verifyRelyingPartyIdHash(env.SERVER_HOST)) return { error: 'Invalid data' };
