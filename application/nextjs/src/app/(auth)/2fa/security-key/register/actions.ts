...
import { registerSecurityKey } from '@acme/backend';
...
export const registerSecurityKeyAction = formAction(schema, async ({ name, attestationObject, clientDataJSON }) => {
  ...
  const result = await registerSecurityKey({ name, ...decoded });
